...
foreach ($Computer in $Computers){
    ...
    if ($NoComputer){
        ...
        $OSCheck = (get-adcomputer $Machine -properties *|select operatingsystem).operatingsystem
        if ($OSCheck -notlike '*SERVER*'){
            ...
            write-host -foregroundcolor magenta "Computer $Computer is a server, ignoring!"
        }
